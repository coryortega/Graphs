...
class Graph:
    ...
    def dft_recursive(self, starting_vertex, visited=set()):
        ...
        """
        # base case, checking if each vertex is in the 'visited' set
        if starting_vertex not in visited:
            # prints each vertex in depth first order
            print(starting_vertex)
            # adds to visited set so we don't print it again
            visited.add(starting_vertex)
            # storings neighboring vertexs in variable
            neighbors = self.get_neighbors(starting_vertex)
            # for each neighor...
            for neighbor in neighbors:
                # recurse through, printing and storing each 
    ...
    def dfs_recursive(self, starting_vertex, destination_vertex, visited=set(), path=[]):
        ...
        if starting_vertex not in visited:
            # redeclare path as itself with the new vertex
            path = path + [starting_vertex]
            # add to visiting so we know we've been there/done that
            visited.add(starting_vertex)
            # if starting_vertex is destination...
            if starting_vertex == destination_vertex:
                # ...STOP RECURSING, RETURN PATH
                return path
            # store neighboring vertex in 'neighbors' 
            neighbors = self.get_neighbors(starting_vertex)
